<template>
    <main>
        <div class="banner">
            <img src="/img/home_banner.jpg">
        </div>

        <div class="container marketing">

            <AppTopUsers :topUsers="hometopUsers"></AppTopUsers>

            <hr class="featurette-divider">

            <CompanyProfile>
                <template #default="slotProfile">
                    <div class="row featurette">
                        <div class="col-md-7">
                            <h2 class="featurette-heading">{{ slotProfile.item.name }} <span class="text-muted">It’ll
                                    blow
                                    your
                                    mind.</span></h2>
                            <p class="lead">Some great placeholder content for the first featurette here. Imagine some
                                exciting
                                prose here.</p>
                        </div>
                        <div class="col-md-5">
                            <svg class="bd-placeholder-img bd-placeholder-img-lg featurette-image img-fluid mx-auto"
                                width="500" height="500" xmlns="http://www.w3.org/2000/svg" role="img"
                                aria-label="Placeholder: 500x500" preserveAspectRatio="xMidYMid slice"
                                focusable="false">
                                <title>Placeholder</title>
                                <rect width="100%" height="100%" fill="#eee"></rect><text x="50%" y="50%" fill="#aaa"
                                    dy=".3em">500x500</text>
                            </svg>

                        </div>
                    </div>

                    <hr class="featurette-divider">

                    <div class="row featurette">
                        <div class="col-md-7 order-md-2">
                            <h2 class="featurette-heading">Oh yeah, it’s that good. <span class="text-muted">See for
                                    yourself.</span></h2>
                            <p class="lead">Another featurette? Of course. More placeholder content here to give you an
                                idea of
                                how this layout would work with some actual real-world content in place.</p>
                        </div>
                        <div class="col-md-5 order-md-1">
                            <svg class="bd-placeholder-img bd-placeholder-img-lg featurette-image img-fluid mx-auto"
                                width="500" height="500" xmlns="http://www.w3.org/2000/svg" role="img"
                                aria-label="Placeholder: 500x500" preserveAspectRatio="xMidYMid slice"
                                focusable="false">
                                <title>Placeholder</title>
                                <rect width="100%" height="100%" fill="#eee"></rect><text x="50%" y="50%" fill="#aaa"
                                    dy=".3em">500x500</text>
                            </svg>

                        </div>
                    </div>
                </template>
            </CompanyProfile>
            <br>
            <br>
        </div>
    </main>
</template>

<script>
import AppTopUsers from "./layouts/AppTopUsers.vue";
import CompanyProfile from "./CompanyProfile.vue";

export default {
    name: "HomePage",
    components: {
        AppTopUsers,
        CompanyProfile
    },

    data() {
        return {
            hometopUsers: [
                {
                    id: 0,
                    name: "Ammar",
                    degree: "BSCS",
                    bio: "",
                    profile: "/img/trainers/trainer-1.jpg",
                },
                {
                    id: 1,
                    name: "Zeeshan",
                    degree: "Civil",
                    bio: "",
                    profile: "/img/trainers/trainer-3.jpg",
                },
                {
                    id: 2,
                    name: "Nabeel",
                    degree: "BS",
                    bio: "",
                    profile: "/img/trainers/trainer-3.jpg",
                },
            ],
        }
    }
}
</script>

<style lang="scss" scoped>
.banner {
    img {
        height: 250px;
        width: 930px;
    }
}

.marketing {
    margin-top: 70px;
}
</style>